...
import { PDFDropzone } from "@/components/pdf-link-genie/PDFDropzone";
import { LinkTable } from "@/components/pdf-link-genie/LinkTable";
import { ExtractionStats } from "@/components/pdf-link-genie/ExtractionStats";
import { ActionButtons } from "@/components/pdf-link-genie/ActionButtons";
import { ProgressBar } from "@/components/pdf-link-genie/ProgressBar";
    ...
    <div className="min-h-screen">
        ...
        {result && (
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              ...
              <ActionButtons links={result.links} selectedLinks={selectedLinks} fileName={result.fileName} onReset={handleReset} onClearSelection={() => setSelectedIds(new Set())} />
            </div>
      ...
      <footer className="border-t border-border mt-auto">
        <div className="container max-w-6xl mx-auto px-4 py-6">
          <p className="text-center text-sm text-muted-foreground">🔒 Your files are processed locally and never uploaded to any server</p>
        </div>
      </footer>
    </div>
  );
