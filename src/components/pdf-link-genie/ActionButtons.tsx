...
export function ActionButtons({ links, selectedLinks, fileName, onReset, onClearSelection }: ActionButtonsProps) {
  ...
  const handleCopy = async () => {
    try {
      ...
      toast.success(`${targetLinks.length} link${targetLinks.length !== 1 ? 's' : ''} copied`);
    } catch { toast.error('Failed to copy links'); }
  };

  const handleExportCSV = () => {
    try { exportToCSV(targetLinks, fileName); toast.success('CSV downloaded'); }
  ...
  const openLinks = () => {
    ...
    if (blocked > 0) toast.error(`${blocked} tabs blocked. Allow popups for this site.`, { duration: 5000 });
    else toast.success(`Opened ${targetLinks.length} links`);
  };
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>You are about to open {targetLinks.length} tabs.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
