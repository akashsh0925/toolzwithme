              ...
              <TableRow className="bg-muted/50">
                <TableHead className="w-12 text-center">
                  <Checkbox
                    ...
                    ref={(el) => { if (el) (el as HTMLButtonElement & { indeterminate?: boolean }).indeterminate = someFilteredSelected; }}
              ...
              {filteredLinks.map((link, index) => (
                <TableRow key={link.id} className={`hover:bg-accent/50 transition-colors ${selectedIds.has(link.id) ? 'bg-accent/30' : ''}`}>
                  <TableCell className="text-center">
                    <Checkbox checked={selectedIds.has(link.id)} onCheckedChange={() => toggleSelection(link.id)} aria-label={`Select link ${index + 1}`} />
                  </TableCell>
